...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Ensure proper chat ID handling
  // Ensure proper chat ID format (prepend with @ for usernames if needed)
  let chatId = process.env.TELEGRAM_CHAT_ID ? 
    (process.env.TELEGRAM_CHAT_ID.startsWith('@') ? process.env.TELEGRAM_CHAT_ID : '@' + process.env.TELEGRAM_CHAT_ID).trim() 
    : '';
  
  // Verify bot and chat ID on startup
  if (process.env.TELEGRAM_BOT_TOKEN && chatId) {
    bot.telegram.getChat(chatId).then(() => {
      console.log("Telegram chat verification successful");
    }).catch(err => {
      console.error("Telegram chat verification failed:", err);
    });
  }
