  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Add to mood board items and make sure the state updates
        setUserMoodBoard(prev => {
          const updatedBoard = [...prev, newItem];
          console.log("Updated mood board:", updatedBoard);
          return updatedBoard;
        });
                  ...
                  >
                    {item.type === 'image' && (
                      <div className="relative h-52">
                        <img 
                          src={`${window.location.origin}${item.content}`}
                    ...
                    {item.type === 'file' && (
                      <div className="h-52 bg-neutral-100 flex items-center justify-center">
                        <div className="text-center p-6">
                          ...
                          <a 
                            href={`${window.location.origin}${item.content}`}
                            target="_blank" 
